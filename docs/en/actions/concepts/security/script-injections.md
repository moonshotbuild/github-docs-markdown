---
source_path: "/en/actions/concepts/security/script-injections"
title: "Script injections"
intro: "Understand the security risks associated with script injections and GitHub Actions workflows."
...
breadcrumbs:
  ...
  - title: "Script injections"
    href: "/en/actions/concepts/security/script-injections"
---
...
When creating workflows, [custom actions](/en/actions/concepts/workflows-and-actions/custom-actions), and [composite actions](/en/actions/tutorials/create-actions/create-a-composite-action), you should always consider whether your code might execute untrusted input from attackers. This can occur when an attacker adds malicious commands and scripts to a context. When your workflow runs, those strings might be interpreted as code which is then executed on the runner.

Attackers can add their own malicious content to the [`github` context](/en/actions/reference/workflows-and-actions/contexts#github-context), which should be treated as potentially untrusted input. These contexts typically end with `body`, `default_branch`, `email`, `head_ref`, `label`, `message`, `name`, `page_name`,`ref`, and `title`. For example: `github.event.issue.title`, or `github.event.pull_request.body`.

You should ensure that these values do not flow directly into workflows, actions, API calls, or anywhere else where they could be interpreted as executable code. By adopting the same defensive programming posture you would use for any other privileged application code, you can help security harden your use of GitHub Actions. For information on some of the steps an attacker could take, see [Secure use reference](/en/actions/reference/security/secure-use#potential-impact-of-a-compromised-runner).
...
For best practices keeping runners secure, see [Secure use reference](/en/actions/reference/security/secure-use#good-practices-for-mitigating-script-injection-attacks).
