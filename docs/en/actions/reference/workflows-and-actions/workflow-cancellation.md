---
source_path: "/en/actions/reference/workflows-and-actions/workflow-cancellation"
title: "Workflow cancellation reference"
intro: "Find information on the steps GitHub takes to cancel a workflow run."
...
breadcrumbs:
  ...
  - title: "Workflow cancellation"
    href: "/en/actions/reference/workflows-and-actions/workflow-cancellation"
---
...
3. For jobs that continue to run, the server re-evaluates `if` conditions for the unfinished steps. If the condition evaluates to `true`, the step continues to run. You can use the `cancelled` expression to apply a status check of `cancelled()`. For more information, see [Evaluate expressions in workflows and actions](/en/actions/reference/workflows-and-actions/expressions#cancelled).
...
5. After the 5 minute cancellation timeout period, the server will forcibly terminate all jobs and steps marked for cancellation that are still running.
