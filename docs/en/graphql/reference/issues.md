...
## addCloseIssueReferences - mutation
...
### Input fields for `addCloseIssueReferences`

* `input` (AddCloseIssueReferencesInput!): 

### Return fields for `addCloseIssueReferences`

* `clientMutationId` (String): A unique identifier for the client performing the mutation.
* `issue` (Issue): The issue the closing references were added to.

## AddCloseIssueReferencesInput - input object

Autogenerated input type of AddCloseIssueReferences.

### Input fields for `AddCloseIssueReferencesInput`

* `clientMutationId` (String): A unique identifier for the client performing the mutation.
* `issueId` (ID!): ID of the issue to add closing references to.
* `pullRequestIds` ([ID!]!): IDs of the pull requests to link as closing references. Limit: 10.
...
## IssueFieldUpdateInput - input object

Updates an issue field using its user-facing name.

### Input fields for `IssueFieldUpdateInput`

* `fieldName` (String!): The name of the issue field.
* `operation` (IssueFieldUpdateOperation!): The operation to perform.
* `value` (String): The value or comma-separated option names for the operation.

## IssueFieldUpdateOperation - enum

The operation to perform on an issue field value.

### Values for `IssueFieldUpdateOperation`

* `ADD`: Sets a scalar field or adds options to a multi-select field.
* `CLEAR`: Clears the field value.
* `REMOVE`: Removes options from a multi-select field.
...
## removeCloseIssueReferences - mutation
...
### Input fields for `removeCloseIssueReferences`

* `input` (RemoveCloseIssueReferencesInput!): 

### Return fields for `removeCloseIssueReferences`

* `clientMutationId` (String): A unique identifier for the client performing the mutation.
* `issue` (Issue): The issue the closing references were removed from.

## RemoveCloseIssueReferencesInput - input object

Autogenerated input type of RemoveCloseIssueReferences.

### Input fields for `RemoveCloseIssueReferencesInput`

* `clientMutationId` (String): A unique identifier for the client performing the mutation.
* `issueId` (ID!): ID of the issue to remove closing references from.
* `pullRequestIds` ([ID!]!): IDs of manually linked pull requests to remove. Auto-detected closing references are left unchanged. Limit: 10.
...
* `issueFieldUpdates` ([IssueFieldUpdateInput!]): Issue field updates resolved by field and option names.
