...
### Values for `PullRequestReviewThreadResolutionReason`

* `ADDRESSED`: The review comment was addressed.
* `INVALID`: The review comment is invalid.
* `WONT_FIX`: The review comment will not be addressed.
...
* `clientMutationId` (String): A unique identifier for the client performing the mutation.
* `resolutionReason` (PullRequestReviewThreadResolutionReason): The reason a Copilot code review thread was resolved.
