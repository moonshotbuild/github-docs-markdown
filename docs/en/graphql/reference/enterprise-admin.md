...
* `adminLogins` ([String!]!): The logins of additional organization owners. Listed users are invited unless
...
* `allowListValue` (String!): An IP address or range of addresses in CIDR notation. Ranges that cover the
...
* `enterpriseTeams` (EnterpriseTeamConnection!): A list of enterprise teams in this enterprise.
  ...
  * `userLogins` ([String!]): User logins to filter by. A maximum of 3 logins can be passed.
