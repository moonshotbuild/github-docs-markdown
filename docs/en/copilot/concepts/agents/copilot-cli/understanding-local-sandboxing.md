---
source_path: "/en/copilot/concepts/agents/copilot-cli/understanding-local-sandboxing"
title: "Understanding filesystem policies for local sandboxing in GitHub Copilot CLI"
intro: "When local sandboxing is enabled, Copilot CLI runs each sandboxed process or operation under a filesystem policy that controls which files and directories it can read and write. Learn how that policy is built and how to check the access it grants."
...
breadcrumbs:
  ...
  - title: "Understanding local sandboxing"
    href: "/en/copilot/concepts/agents/copilot-cli/understanding-local-sandboxing"
---
...
To run a program such as `python` or `git`, the sandbox has to let the command see the directory the program lives in. When **Allow dev tool access** is enabled—as it is by default—Copilot grants **read-only** access to the directories listed in your `PATH` environment variable, along with directories named by related tool variables such as `GOPATH`, `JAVA_HOME`, and `PYTHONPATH`. Read-only is the right level for external tools: a command needs to run `git`, not modify it. If you turn **Allow dev tool access** off, these directories are no longer granted automatically and must come from your own allow rules. For the complete list of the `PATH` and toolchain environment variables the sandbox inspects, and how each one is interpreted, see [GitHub Copilot CLI command reference](/en/copilot/reference/copilot-cli-reference/cli-command-reference#sandbox-tool-directory-grants).
