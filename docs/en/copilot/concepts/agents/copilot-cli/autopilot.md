...
* **Safety:** Autopilot mode allows Copilot to take multiple self-directed steps to finish your task. By default, it pauses after 5 continuation messages; use `--max-autopilot-continues` to change this limit and avoid infinite loops. Also, in autopilot mode, Copilot cannot carry out any actions that require permission unless you explicitly grant it full permissions.
