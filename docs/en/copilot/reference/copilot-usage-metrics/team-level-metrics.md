---
source_path: "/en/copilot/reference/copilot-usage-metrics/team-level-metrics"
title: "Team-level Copilot usage metrics"
intro: "Construct team-level GitHub Copilot usage metrics by joining the daily user-teams report with the daily per-user usage metrics report."
...
breadcrumbs:
  ...
  - title: "Team-level metrics"
    href: "/en/copilot/reference/copilot-usage-metrics/team-level-metrics"
---
...
For complete request and response schemas, authentication requirements, and related endpoints, see [REST API endpoints for Copilot usage metrics](/en/rest/copilot/copilot-usage-metrics). For an overview of how individual usage metric fields are defined, see [Data available in Copilot usage metrics](/en/copilot/reference/copilot-usage-metrics/copilot-usage-metrics).
...
For the full list of fields available in these reports, see [Data available in Copilot usage metrics](/en/copilot/reference/copilot-usage-metrics/copilot-usage-metrics).
...
| team\_id | slug     | active\_users | code\_acceptance\_activity\_count | loc\_added\_sum |
| :------- | :------- | :------------ | :-------------------------------- | :-------------- |
...
2. **Inner-join the two reports** on `(user_id, day, entity_id)`. All three keys must match. The join is one-to-many on the team side—a user on multiple teams matches multiple user-teams rows.

3. **Filter by `day`** to the day you want. Both reports carry the same `day` value.

4. **Group by `team_id`** (and `slug` for the team's display name) and aggregate. Use:
...
### Cutting by agent app

Usage for each agent app is in the `totals_by_3rd_party_agent` array on each per-user row. To create team-level agent app metrics, expand the array after joining the reports, group by `team_id` and `agent_id`, and sum the nested `user_initiated_interaction_count`. Use `agent_id` as the stable grouping key and `agent_name` for display.

Per-user agent app entries do not include `session_count`, so you cannot derive team-level session counts from the user-teams join.
...
2. Join each day's per-user usage metrics report (`organization_users_1_day` or `users_1_day`) with the same day's user-teams report (`organization_user_teams_1_day` or `enterprise_user_teams_1_day`) on `(user_id, day, entity_id)`.
3. Filter `day` to the window and drop `day` from the grouping.
...
* **More feature surfaces are tracked.** The volume counters (`code_generation_activity_count`, `code_acceptance_activity_count`, and the `loc_*` counters) aggregate activity across multiple Copilot surfaces—inline IDE completions, chat panel actions, and (for accepted-line counters) Copilot agent edits. For per-counter surface coverage details, see [Data available in Copilot usage metrics](/en/copilot/reference/copilot-usage-metrics/copilot-usage-metrics). If you previously consumed similar metrics from a surface that counted only inline IDE completions, expect higher values in these counters and re-baseline rather than diffing across the cutover.
...
* For the full schema and field reference for the per-user usage metrics report, see [Data available in Copilot usage metrics](/en/copilot/reference/copilot-usage-metrics/copilot-usage-metrics).
* For example JSON payloads from the usage metrics endpoints, see [Example schema for Copilot usage metrics](/en/copilot/reference/copilot-usage-metrics/example-schema).
* For guidance on reconciling metrics across the dashboard, APIs, and exports, see [Reconciling Copilot usage metrics across dashboards, APIs, and reports](/en/copilot/reference/copilot-usage-metrics/reconciling-usage-metrics).
