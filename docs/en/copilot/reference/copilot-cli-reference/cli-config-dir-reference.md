...
If `settings.json` contains a top-level key that isn't a recognized setting (for example, a typo), Copilot CLI shows a startup warning listing the unknown key(s)—up to five, with a count of any more—and ignores them since they have no effect. `$schema` is tolerated and never reported.
