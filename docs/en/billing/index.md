---
source_path: "/en/billing"
title: "Billing and payments documentation"
intro: "Learn about the different components of your bill, and how you can view and manage those components."
product: "Billing and payments"
document_type: "product"
breadcrumbs:
  - title: "Billing and payments"
    href: "/en/billing"
---

# Billing and payments documentation

Learn about the different components of your bill, and how you can view and manage those components.

## Recommended

* [Viewing your usage of metered products and licenses](/en/billing/how-tos/products/view-productlicense-use)

  Explore your use of features that are billed by usage and see how they contribute to your bill.

## Links

### Getting started

* [Billing for GitHub Enterprise](/en/billing/concepts/enterprise-billing/billing-for-enterprises)

  Understand what makes up your enterprise bill so you can better forecast and manage costs.

## Articles

* [How GitHub billing works](/en/billing/get-started/how-billing-works)

  Learn what you'll be charged for, when charges occur, and how to track your usage on GitHub to avoid billing surprises.

* [Introduction to billing and licensing](/en/billing/get-started/introduction-to-billing)

  Learn about the billing platform's key functionalities, and how they can help you manage your spending more effectively.

* [Billing manager onboarding](/en/billing/get-started/billing-manager-onboard)

  A quick overview of your role and how you can work with organization and enterprise owners to control and track costs in GitHub.

* [Billing cycles](/en/billing/concepts/billing-cycles)

  Learn about the billing cycles for self-serve, Azure, and invoiced payment methods.

* [Budgets and alerts](/en/billing/concepts/budgets-and-alerts)

  Budgets help you track and control spending on different products.

* [Cost centers](/en/billing/concepts/cost-centers)

  Attribute spending to specific parts of your business.

* [Azure subscription payments](/en/billing/concepts/azure-subscriptions)

  Learn about paying for metered usage of GitHub plans, licenses, and usage with an Azure subscription.

* [Impact of changing your plan on billing](/en/billing/concepts/impact-of-plan-changes)

  Learn how upgrading or downgrading your plan is reflected in billing.

* [Discounted plans for GitHub accounts](/en/billing/concepts/discounted-plans)

  GitHub provides discounts to students, educators, educational institutions, nonprofits, and libraries.

* [Billing for GitHub Enterprise](/en/billing/concepts/enterprise-billing/billing-for-enterprises)

  Understand what makes up your enterprise bill so you can better forecast and manage costs.

* [Usage-based billing for enterprise licenses](/en/billing/concepts/enterprise-billing/usage-based-licenses)

  Learn about usage-based billing for licenses in your GitHub Enterprise plan, whether you pay through GitHub or Azure.

* [Combined GitHub Enterprise cloud and server use](/en/billing/concepts/enterprise-billing/combined-enterprise-use)

  Your enterprise account enables you to set up GitHub Enterprise Server with no additional cost.

* [License files for GitHub Enterprise Server](/en/billing/concepts/enterprise-billing/ghes-license-files)

  To use GitHub Enterprise Server, you must upload a license file.

* [About Visual Studio subscriptions with GitHub Enterprise](/en/visual-studio-subs)

  You can give Visual Studio subscribers on your team access to GitHub Enterprise with a combined offering from Microsoft.

* [Combined use of GitHub Enterprise and Azure DevOps](/en/billing/concepts/enterprise-billing/azure-devops-licenses)

  GitHub Enterprise Cloud customers who manage users with Entra ID can access Azure DevOps without additional cost.

* [GitHub Actions billing](/en/billing/concepts/product-billing/github-actions)

  Learn how usage of GitHub Actions is measured against your free allowance and how to pay for additional use.

* [GitHub Advanced Security license billing](/en/billing/concepts/product-billing/github-advanced-security)

  Learn how usage of Advanced Security features is measured and how to pay for additional licenses.

* [GitHub Codespaces billing](/en/billing/concepts/product-billing/github-codespaces)

  Learn about the costs for using GitHub Codespaces, and the monthly usage quotas included with GitHub personal accounts.

* [GitHub Code Quality billing](/en/billing/concepts/product-billing/github-code-quality)

  In addition to standard GitHub Actions minutes, GitHub Code Quality billing has two parts: a per-committer license and AI credit usage for AI-powered features.

* [GitHub Copilot billing](/en/billing/concepts/product-billing/github-copilot-billing)

  Copilot usage is measured and billed according to your plan.

* [GitHub Copilot licenses](/en/billing/concepts/product-billing/github-copilot-licenses)

  Learn how licenses for Copilot work, including usage measurement and managing your budget.

* [GitHub Packages billing](/en/billing/concepts/product-billing/github-packages)

  Learn how usage of GitHub Packages is measured against your free allowance and how to pay for additional use.

* [Billing for cloud and local sandboxes for GitHub Copilot](/en/billing/concepts/product-billing/cloud-and-local-sandboxes)

  Learn how usage of Copilot cloud and local sandboxes is measured and billed.

* [Git Large File Storage billing](/en/billing/concepts/product-billing/git-lfs)

  Learn how usage of Git Large File Storage is measured against your free allowance and how to pay for additional use.

* [GitHub Spark billing](/en/billing/concepts/product-billing/github-spark)

  Learn how GitHub Spark is billed for users.

* [GitHub Sponsors billing](/en/billing/concepts/third-party-payments/github-sponsors)

  Understand how sponsorship payments appear in your billing, how they align with your existing payment method and billing date, and what fees apply.

* [GitHub Marketplace app subscriptions](/en/billing/concepts/third-party-payments/github-marketplace-apps)

  Understand how billing works for paid GitHub Marketplace apps, including shared billing date and payment method, proration, free trials, unit limits, and plan changes.

* [Managing your payment and billing information](/en/billing/how-tos/set-up-payment/manage-payment-info)

  Learn how to view, modify, and remove your payment information and billing contacts using the new billing platform.

* [Connecting an Azure subscription](/en/billing/how-tos/set-up-payment/connect-azure-sub)

  You can enable and pay for usage-based billing on GitHub by connecting an Azure subscription.

* [Setting up Visual Studio subscriptions with GitHub Enterprise](/en/set-up-vs-subscription)

  Your team's subscription to Visual Studio can also provide access to GitHub Enterprise.

* [Redeeming a coupon](/en/billing/how-tos/set-up-payment/redeem-coupon)

  If you have a coupon, you can redeem it towards a paid GitHub subscription.

* [Adding a sales tax certificate](/en/billing/how-tos/set-up-payment/add-sales-tax-certificate)

  If you're a customer in the United States with a GitHub Customer Agreement and you're exempt from sales tax, you can upload a certificate to ensure the correct sales tax amount is calculated.

* [Managing invoices for your enterprise](/en/billing/how-tos/set-up-payment/manage-enterprise-invoice)

  You can view, pay, or download a current invoice for your enterprise, and you can view your payment history.

* [Payments for customers in India](/en/billing/how-tos/set-up-payment/india-one-time-payments)

  Customers in India who have been impacted by the Reserve Bank of India's recurring payment regulation can use RBI-compliant payment mandates with eligible credit card payment methods or make one-time payments for their GitHub subscriptions and services.

* [Upgrading your account's plan](/en/billing/how-tos/manage-plan-and-licenses/upgrade-plan)

  You can upgrade the plan for a personal account or organization on GitHub at any time.

* [Downgrading your account's plan](/en/billing/how-tos/manage-plan-and-licenses/downgrade-plan)

  You can downgrade the plan for any type of account on GitHub at any time.

* [Viewing and managing pending changes to your plan](/en/billing/how-tos/manage-plan-and-licenses/manage-pending-changes)

  You can view and cancel pending changes to your plan before they take effect on your next billing date.

* [Viewing usage for your GitHub Enterprise plan](/en/billing/how-tos/manage-plan-and-licenses/view-enterprise-usage)

  Learn how to display the number of users in your enterprise. For GitHub Enterprise Cloud full billing information is also available.

* [Managing user licenses for an organization or enterprise](/en/billing/how-tos/manage-plan-and-licenses/manage-user-licenses)

  Add licenses to allow more people to join your organization or enterprise.

* [Setting up budgets to control spending on metered products](/en/billing/how-tos/set-up-budgets)

  Prevent overspending by setting budgets and tracking when metered usage nears or exceeds a budget threshold.

* [Viewing your usage of metered products and licenses](/en/billing/how-tos/products/view-productlicense-use)

  Explore your use of features that are billed by usage and see how they contribute to your bill.

* [Downloading license use for your enterprise or organization](/en/billing/how-tos/products/download-license-use)

  Get data on consumption of GitHub, Copilot, and Advanced Security licenses.

* [Buying Advanced Security for your organization or enterprise](/en/billing/how-tos/products/buy-advanced-security)

  How to buy licenses for GitHub Advanced Security whether you have usage-based or volume/subscription billing.

* [Viewing and estimating your spending](/en/billing/how-tos/products/estimate-spending)

  View current usage and spending trends across paid GitHub products, so you can estimate costs and make informed budgeting decisions.

* [Using cost centers to allocate costs to business units](/en/billing/how-tos/products/use-cost-centers)

  Learn how to create and use cost centers to manage costs across your company's divisions at scale.

* [Managing volume licenses for GitHub Advanced Security](/en/billing/how-tos/products/manage-ghas-licenses)

  You can monitor and control the availability and consumption of licenses for Advanced Security in repositories in your enterprise.

* [Viewing committer information for Advanced Security licenses](/en/view-ghas-committers)

  You can view information about the GitHub Secret Protection and GitHub Code Security committers for your enterprise and calculate the cost for additional committers with the site admin dashboard.

* [Disabling GitHub Advanced Security for your enterprise](/en/billing/how-tos/products/disable-ghas-for-enterprise)

  Disable GitHub Advanced Security and prevent accidental re-enablement across your enterprise.

* [Manage Enterprise Server licenses for your enterprise account](/en/manage-server-licenses)

  Learn how to set up an instance of GitHub Enterprise Server that synchronizes license information with your enterprise account on GitHub Enterprise Cloud.

* [Upgrading the billing plan for a GitHub Marketplace app](/en/billing/how-tos/pay-third-parties/upgrade-marketplace-app)

  You can upgrade your GitHub Marketplace app to a different plan at any time.

* [Downgrading the billing plan for a GitHub Marketplace app](/en/billing/how-tos/pay-third-parties/downgrade-marketplace-app)

  If you'd like to use a different billing plan, you can downgrade your GitHub Marketplace app at any time.

* [Canceling a GitHub Marketplace app](/en/billing/how-tos/pay-third-parties/cancel-marketplace-app)

  You can cancel and remove a GitHub Marketplace app from your account at any time.

* [Upgrading a sponsorship](/en/billing/how-tos/pay-third-parties/upgrade-sponsorship)

  You can upgrade your sponsorship to a higher tier.

* [Downgrading a sponsorship](/en/billing/how-tos/pay-third-parties/downgrade-sponsorship)

  You can downgrade your sponsorship to a lower tier.

* [Canceling a sponsorship](/en/billing/how-tos/pay-third-parties/cancel-sponsorship)

  You can cancel your sponsorship.

* [Setting an end date to a sponsorship](/en/billing/how-tos/pay-third-parties/end-sponsorship)

  You can set an end date to an existing or new sponsorship.

* [Creating and paying for an organization for a client](/en/billing/how-tos/manage-for-client/create-client-org)

  You can create and pay for a GitHub organization on behalf of a client.

* [Creating and paying for a client's enterprise account](/en/billing/how-tos/manage-for-client/create-client-enterprise)

  You can create and pay for an enterprise account on GitHub on behalf of a client.

* [Creating an enterprise account as a Microsoft CSP partner](/en/billing/how-tos/manage-for-client/create-as-csp-partner)

  Learn how to set up an enterprise account for your customer as a Microsoft Cloud Solution Provider partner.

* [Managing your client's paid organization](/en/billing/how-tos/manage-for-client/manage-client-org)

  Billing managers can upgrade or downgrade a client's paid organization at any time, and set their plan to renew.

* [Troubleshooting a declined credit card charge](/en/billing/how-tos/troubleshooting/declined-card)

  If the credit card you use to pay for GitHub is declined, you can take several steps to ensure that your payments go through and that you are not locked out of your account.

* [Unlocking a locked account](/en/billing/how-tos/troubleshooting/locked-account)

  Your account's paid features are locked if your payment is past due because of billing problems.

* [Troubleshooting Azure subscription connection problems](/en/billing/how-tos/troubleshooting/azure-sub-connection)

  Tips for resolving some common issues with connection of an Azure subscription to your account on GitHub.

* [Troubleshooting license usage for GitHub Enterprise](/en/billing/how-tos/troubleshooting/enterprise-license-usage)

  You can troubleshoot license usage for your enterprise by auditing license reports.

* [Actions runner pricing](/en/billing/reference/actions-runner-pricing)

  Reference information for calculating the cost of using different GitHub-hosted runners.

* [Billing through Azure subscriptions](/en/billing/reference/azure-billing)

  Learn how billing works when you connect your GitHub account to an Azure subscription.

* [Azure subscription reference](/en/billing/reference/azure-subscription)

  Reference information for connecting an Azure subscription to pay for usage of GitHub.

* [Billing reports reference](/en/billing/reference/billing-reports)

  Billing reports show detailed GitHub usage, AI credits consumption, and billing information for your account.

* [Roles for the billing platform](/en/billing/reference/billing-roles)

  Learn about the different roles that can view and manage billing settings.

* [Cost center allocation for different products](/en/billing/reference/cost-center-allocation)

  Learn how your spending is assigned to cost centers for licensed and metered products.

* [License troubleshooting information for GitHub Enterprise](/en/enterprise-license-troubleshooting)

  Troubleshoot license usage for your enterprise by understanding consumption criteria and auditing license reports.

* [People who consume a license in an organization](/en/billing/reference/github-license-users)

  Learn how consumption of GitHub licenses is determined for paid organizations and enterprises.

* [License reports reference](/en/billing/reference/license-reports)

  License reports show details of the users consuming licenses that you pay for.

* [GitHub Product and SKU names](/en/billing/reference/product-and-sku-names)

  Learn about the product and SKU identifiers used in the billing platform and REST API.

* [Product usage included with each plan](/en/billing/reference/product-usage-included)

  Check the size of the allowance for usage-based products included in each GitHub plan.

* [Roles for Visual Studio subscriptions with GitHub Enterprise reference](/en/roles-for-visual-studio)

  Learn about the different roles required to set up Visual Studio subscriptions with GitHub Enterprise.

* [Supported payment methods for GitHub](/en/billing/reference/supported-payment-methods)

  Reference information detailing the supported payment methods for GitHub.

* [Automating usage reporting with the REST API](/en/billing/tutorials/automate-usage-reporting)

  Learn how to automate reporting on your use of paid features using the REST API.

* [Start monitoring costs with soft budgets](/en/billing/tutorials/soft-budgets)

  Set soft budgets to monitor spending without blocking access to paid features while you build an understanding of your costs.

* [Controlling and tracking costs at scale](/en/billing/tutorials/control-costs-at-scale)

  Control costs and provide granular reporting for your enterprise by mapping your company's financial structures to cost centers and setting budgets at scale.

* [Gathering insights on your spending](/en/billing/tutorials/gather-insights)

  Get insights into the usage of your  members.
