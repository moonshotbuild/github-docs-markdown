...
The specific metadata available depends on what the service provider shares with GitHub. Not all secret types support extended metadata checks. For more information, see [Evaluating alerts from secret scanning](/en/code-security/tutorials/remediate-leaked-secrets/evaluating-alerts#verifying-token-metadata).
